    ...
    for item in items:
        try:
            if not isinstance(item, dict):
                raise ValueError("Invalid item format. Each item must be a dictionary.")
            if "price" not in item or "quantity" not in item:
                raise ValueError("Invalid item format. Each item must contain 'price' and 'quantity' keys.")
